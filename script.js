...
<body>
    ...
    createFile(name, type) {
        if (!this.currentProject || !this.currentFolder || !name.trim() || !type) return null;
...
class DOMManager {
    ...
    updateButtonStates() {
        ...
        const hasFolder = hasProject && !!this.state.currentFolder;
        
        this.addFolderBtn.disabled = !hasProject;
        this.addFileBtn.disabled = !hasFolder;
        this.uploadBtn.disabled = !hasFolder;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize with first project if exists
    const projectIds = Object.keys(state.projects);
    if (projectIds.length > 0) {
        state.switchProject(projectIds[0]);
        domManager.loadFile(state.currentFile);
    }
});
